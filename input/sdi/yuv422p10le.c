 ...
 * 0,0 is top left.
 */
uint16_t *yuv422p10le_get_luma_position(uint16_t *luma, unsigned int x, unsigned int y, unsigned int frame_width_pixels, unsigned int frame_height_pixels)
{
        if (x >= frame_width_pixels)
                y = frame_width_pixels - 1;
        if (y >= frame_height_pixels)
                y = frame_height_pixels - 1;

        uint16_t *ptr = luma + x + (frame_width_pixels * y);
        return ptr;
}

void yuv422p10le_draw_black_pixel(uint16_t *luma, unsigned int x, unsigned int y, unsigned int frame_width_pixels, unsigned int frame_height_pixels)
{
        uint16_t *pos = yuv422p10le_get_luma_position(luma, x, y, frame_width_pixels, frame_height_pixels);
        *pos = 0x0;
}

void yuv422p10le_draw_white_pixel(uint16_t *luma, unsigned int x, unsigned int y, unsigned int frame_width_pixels, unsigned int frame_height_pixels)
{
        uint16_t *pos = yuv422p10le_get_luma_position(luma, x, y, frame_width_pixels, frame_height_pixels);
        *pos = 0x3ff;
}

void yuv422p10le_draw_black_32x32_box(uint16_t *luma, unsigned int x, unsigned int y, unsigned int frame_width_pixels, unsigned int frame_height_pixels)
{
        for (int i = 0; i < 32; i++) {
                for (int j = 0; j < 32; j++) {
                        yuv422p10le_draw_black_pixel(luma, x + i, y + j, frame_width_pixels, frame_height_pixels);
                }
        }
}

void yuv422p10le_draw_white_32x32_box(uint16_t *luma, unsigned int x, unsigned int y, unsigned int frame_width_pixels, unsigned int frame_height_pixels)
{
        for (int i = 0; i < 32; i++) {
                for (int j = 0; j < 32; j++) {
                        yuv422p10le_draw_white_pixel(luma, x + i, y + j, frame_width_pixels, frame_height_pixels);
                }
        }
}
#endif
...
{
	for (int i = 0; i < (YUV422P10LE_BOX_HEIGHT / 6); i++) {
		addr[0] = coloring[0];
		addr[1] = coloring[1];
		addr[2] = coloring[2];
		addr[3] = coloring[3];
...
void YUV422P10LE_draw_box(uint32_t *frame_addr, uint32_t stride, int color, int interlaced)
{
	uint32_t *coloring;
	if (color == 1)
		coloring = V210_white;
	...
	interleaved = 1;
	for (uint32_t l = 0; l < YUV422P10LE_BOX_HEIGHT; l++) {
		uint32_t *addr = frame_addr + ((l * interleaved) * (stride / 4));
		YUV422P10LE_draw_6_pixels(addr, coloring);
	}
}
...
__inline__ void YUV422P10LE_draw_box_at(uint32_t *frame_addr, uint32_t stride, int color, int x, int y, int interlaced)
{
	...
	addr += ((x / 6) * 4);
	YUV422P10LE_draw_box(addr, stride, color, interlaced);
}
...
void YUV422P10LE_write_32bit_value(void *frame_bytes, uint32_t stride, uint32_t value, uint32_t lineNr, int interlaced)
{
	for (int p = 31, sh = 0; p >= 0; p--, sh++) {
		YUV422P10LE_draw_box_at(((uint32_t *)frame_bytes), stride,
			(value & (1 << sh)) == (uint32_t)(1 << sh), p * YUV422P10LE_BOX_HEIGHT, lineNr, interlaced);
	}
}

uint32_t YUV422P10LE_read_32bit_value(void *frame_bytes, uint32_t stride, uint32_t lineNr, double scalefactor)
{
	double pixheight = YUV422P10LE_BOX_HEIGHT * scalefactor;
...
static int YUV422P10LE_render_character(struct YUV422P10LE_painter_s *ctx, unsigned char letter)
{
	...
	for (int i = 0; i < 8; i++) { /* Row */
                ...
                ctx->ptr += (ctx->widthPixels * 2);
	}
    
	return 0;
}

static void YUV422P10LE_painter_moveto(struct YUV422P10LE_painter_s *ctx, int x, int y)
{
	ctx->ptr = ctx->frame + (x * 6 * 2);
	ctx->ptr += (y * 6 * 4 * ctx->widthPixels);
}

static int YUV422P10LE_render_character_at(struct YUV422P10LE_painter_s *ctx, unsigned char letter, int x, int y)
{
        if (letter > 0x9f)
                return -1;

        YUV422P10LE_painter_moveto(ctx, x, y);
        YUV422P10LE_render_character(ctx, letter);
...
void YUV422P10LE_painter_draw_ascii_at(struct YUV422P10LE_painter_s *ctx, int x, int y, const char *str)
{
	for (int i = 0; i < strlen(str); i++)
		YUV422P10LE_render_character_at(ctx, *(str + i), x + i, y);
}
